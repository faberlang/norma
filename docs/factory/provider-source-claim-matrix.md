...
- `aleator`, `consolum`, `processus`, `solum`, and `tempus` may be described as
  ...
  `host-providers-rs` commit `ed34a67`, but not as public runnable support.
...
| Local provider-coverage evidence | A repo-local provider packet, such as `host-providers-rs` `ed34a67`, shows manifest/dispatch agreement and local tests for the route family. Norma's route audit must also pass against the sibling provider workspace at the expected revision, including its missing-dispatch and revision-mismatch negative self-tests. This permits private-preview evidence wording only. |
