...
| Compile/import-evidenced native helpers | `csv`, `json`, `json/solve`, `json/cursor`, `json/lexica`, `model`, `tensor`, `valor`, `vector`, `solum/path`; native portions of `chorda` and `tempus` | `faber script scripta/check-promoted-helper-imports.fab` runs `faber check` import smoke for each listed `norma:*` module using this repo as `FABER_LIBRARY_HOME` | These helper surfaces have local compile/import evidence as source modules | Runtime product behavior, complete module behavior, or public run claims without focused run evidence |
...
| Compile-gap native helper surfaces | `mathesis`; `json/pange` | Native source exists, but current import smoke fails before this packet's promotion threshold; keep out of promoted rows until a focused compiler/source fix lands and `check-promoted-helper-imports` covers it. `json/pange` re-tiered here 2026-08-09 (SEM016; head-cto disposition on need 0a1812a5): check-mode `faber check` rejects the compiler-owned `json`→`textus` conversio (`unsupported_conversio`; `Primitive::Json` is family Other; `--permissive` does not downgrade); script/run mode works via the runtime codec (kernel `faber:json.pange`). Severity P2; soft_gate (governance classification, not a runtime defect). Owner: radix compiler conversio surface (alternate closure: `pange.fab` runtime-codec dispatch rewrite). Recheck predicate: flip to green when the compiler conversio surface supports json→textus at typecheck, OR pange.fab is rewritten to dispatch through the runtime codec. | Source shape and local audit only | Compile/import-evidenced helper or runtime behavior claim |
