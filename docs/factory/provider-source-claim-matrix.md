...
| Local provider-coverage evidence | A repo-local provider packet, such as `host-providers-rs` `fc08be5`, shows manifest/dispatch agreement and local tests for the route family. Norma's route audit must also pass against the sibling provider workspace, including its missing-dispatch negative self-test. This permits private-preview evidence wording only. |
