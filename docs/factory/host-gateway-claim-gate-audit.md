...
| `host-providers-rs` | Commit `ed34a67` records local manifest/dispatch coverage for `aleator` 5 routes, `consolum` 16, `processus` 10 provider-covered routes, `solum` 45, and `tempus` 4. `./scripta/audit-provider-route-claims` now verifies the same sibling-workspace route contract at the expected provider revision: Norma source `ad` routes match provider manifests, and every manifest route appears in the matching Rust dispatch table. `processus:exi` is intentionally unmanifested and rejected until host exit has a protocol-visible terminal response. | Needs package/export linkage, public contract generation, and public example run evidence before support claims. |
